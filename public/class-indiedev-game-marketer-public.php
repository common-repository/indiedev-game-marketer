...
class Indiedev_Game_Marketer_Public {
	...
	public function enqueue_scripts() {
        ...
        public function Indiedev_Game_Marketer_promo_posttype() {
            ...
            register_post_type( 'promo_materials',
                    array(
                            ...
                            'supports' => array('title', 'editor', 'revisions', 'comments', 'custom-fields', 'trackbacks', 'author', 'thumbnail', 'excerpt' ),
                            'public' => true,
                            'has_archive' => false,
        ...
        public function fix_link($link_or_zelda_to_fix, $tolower=true) {
        	if($tolower) {
            	$fixed = strtolower($link_or_zelda_to_fix);
			} else {
				$fixed = $link_or_zelda_to_fix;
			}
        ...
        public function idgm_display_presskit($game_id) {
            ...
            if (trim($the_game['logo'])!='' || trim($the_game['icon'])!='') {
                ...
                if($the_logo) {
                    $return_value .= '<img src="'.$this->fix_link($the_game['logo'], false).'" alt="'.htmlentities($the_game['name']).' '.__('Logo', 'indiedev-game-marketer').'" />';
                }           
                if($the_icon) {
                    $return_value .= '<img src="'.$this->fix_link($the_game['icon'], false).'" alt="'.htmlentities($the_game['name']).' '.__('Icon', 'indiedev-game-marketer').'" />';
                }   
                $return_value .= '</div><br />';
            ...
            if(trim($the_game['community_managers'])!='') {
                $return_value .= "<li class='indiedev-list-item'><strong>".__('Community Managers', 'indiedev-game-marketer'). "</strong>: " . $the_game['community_managers'] . "</li>";
            }  
            if(trim($the_game['pr_managers'])!='') {
                $return_value .= "<li class='indiedev-list-item'><strong>".__('PR Managers', 'indiedev-game-marketer'). "</strong>: " . $the_game['pr_managers'] . "</li>";
            }  
            if(trim($the_game['social_media_manager'])!='') {
                $return_value .= "<li class='indiedev-list-item'><strong>".__('Social Media Managers', 'indiedev-game-marketer'). "</strong>: " . $the_game['social_media_manager'] . "</li>";
            }  						         
            $return_value .= "</ul>";
            $return_value .= '</div>';            
        ...
        public function indiedev_game_marketer_shortcode($attribs) {
            ...
            } elseif (strtolower($shortcode_attributes['display']) == 'name' && $shortcode_attributes['game'] != 0 ) {
